...
from autotabular.pipeline.components.feature_preprocessing.no_preprocessing import NoPreprocessing
from autotabular.pipeline.components.feature_preprocessing.pca import PCA
from autotabular.pipeline.components.feature_preprocessing.random_trees_embedding import RandomTreesEmbedding
from autotabular.pipeline.components.feature_preprocessing.truncatedSVD import TruncatedSVD
...
class TestCreateClassificationSearchspace(unittest.TestCase):
    ...
    def test_get_match_array_sparse_and_dense(self):
        ...
        # Dense
        m = autotabular.pipeline.create_searchspace_util.get_match_array(
            pipeline=((0, PCA), (1, LDA)), dataset_properties={'sparse': True})
        self.assertEqual(numpy.sum(m), 0)

        m = autotabular.pipeline.create_searchspace_util.get_match_array(
            pipeline=((0, PCA), (1, LDA)),
        ...
        m = autotabular.pipeline.create_searchspace_util.get_match_array(
            pipeline=((0, Preprocessors), (1, Preprocessors), (2,
                                                               Classifiers)),
            dataset_properties={'sparse': False})
    ...
    def test_add_forbidden(self):
        ...
        new_cs = autotabular.pipeline.create_searchspace_util.add_forbidden(
            conf_space=cs,
