...
from autotabular.pipeline.components.data_preprocessing.category_encoders.helmert_encoder import HelmertEncoderTransformer
...
class HelmertEncoderTransformerTest(unittest.TestCase):
    ...
    def test_data_type_consistency(self):
        ...
        Y = HelmertEncoderTransformer().fit_transform(X)
    ...
    def test_default_configuration(self):
        transformations = []
        for i in range(2):
            configuration_space = HelmertEncoderTransformer.get_hyperparameter_search_space(
            )
            default = configuration_space.get_default_configuration()

            preprocessor = HelmertEncoderTransformer(
                random_state=1,
                **{
