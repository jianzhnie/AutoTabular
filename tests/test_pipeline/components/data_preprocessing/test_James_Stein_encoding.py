...
from autotabular.pipeline.components.data_preprocessing.category_encoders.James_Stein_encoder import JamesSteinEncoderTransformer
...
class JamesSteinEncoderTransformerTest(unittest.TestCase):
    ...
    def test_data_type_consistency(self):
        ...
        Y = JamesSteinEncoderTransformer().fit_transform(X, y)
        self.assertFalse(sparse.issparse(Y))

        # X = sparse.csc_matrix(([3, 6, 4, 5], ([0, 1, 2, 1], [3, 2, 1, 0])),
        #                       shape=(3, 4))
        # Y = JamesSteinEncoderTransformer().fit_transform(X)
        # self.assertTrue(sparse.issparse(Y))

    def test_default_configuration(self):
        transformations = []
        for i in range(2):
            configuration_space = JamesSteinEncoderTransformer.get_hyperparameter_search_space(
            )
            default = configuration_space.get_default_configuration()

            preprocessor = JamesSteinEncoderTransformer(
                random_state=1,
                **{
