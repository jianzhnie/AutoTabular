...
from autotabular.pipeline.components.data_preprocessing.category_encoders.catboost_encoder import CatBoostEncoderTransformer
...
class CatBoostEncoderTransformerTest(unittest.TestCase):
    ...
    def test_data_type_consistency(self):
        ...
        Y = CatBoostEncoderTransformer().fit_transform(X, y)
        self.assertFalse(sparse.issparse(Y))

        # X = sparse.csc_matrix(([3, 6, 4, 5], ([0, 1, 2, 1], [3, 2, 1, 0])),
        #                       shape=(3, 4))
        # Y = BaseNEncoderTransformer().fit_transform(X)
        # self.assertTrue(sparse.issparse(Y))

    def test_default_configuration(self):
        transformations = []
        for i in range(2):
            configuration_space = CatBoostEncoderTransformer.get_hyperparameter_search_space(
            )
            default = configuration_space.get_default_configuration()

            preprocessor = CatBoostEncoderTransformer(
                random_state=1,
                **{
    ...
    #     for i in range(2):
    #         configuration_space = BDEncoderTransformer.get_hyperparameter_search_space(
    #         )
    #         default = configuration_space.get_default_configuration()

    #         preprocessor = BDEncoderTransformer(
    #             random_state=1,
    #             **{
    #                 hp_name: default[hp_name]
    #                 for hp_name in default if default[hp_name] is not None
