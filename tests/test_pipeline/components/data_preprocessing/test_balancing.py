...
from autotabular.pipeline.components.classification.adaboost import AdaboostClassifier
from autotabular.pipeline.components.classification.decision_tree import DecisionTree
from autotabular.pipeline.components.classification.extra_trees import ExtraTreesClassifier
from autotabular.pipeline.components.classification.gradient_boosting import GradientBoostingClassifier
from autotabular.pipeline.components.classification.liblinear_svc import LibLinear_SVC
from autotabular.pipeline.components.classification.libsvm_svc import LibSVM_SVC
from autotabular.pipeline.components.classification.passive_aggressive import PassiveAggressive
from autotabular.pipeline.components.classification.random_forest import RandomForest
from autotabular.pipeline.components.classification.sgd import SGD
from autotabular.pipeline.components.data_preprocessing.balancing.balancing import Balancing
from autotabular.pipeline.components.feature_preprocessing.extra_trees_preproc_for_classification import ExtraTreesPreprocessorClassification
from autotabular.pipeline.components.feature_preprocessing.liblinear_svc_preprocessor import LibLinear_Preprocessor
...
class BalancingComponentTest(unittest.TestCase):
    ...
    def test_balancing_get_weights_treed_multilabel(self):
        Y = np.array([[0, 0, 0]] * 100 + [[1, 0, 0]] * 100 +
                     [[0, 1, 0]] * 100 + [[1, 1, 0]] * 100 +
                     [[0, 0, 1]] * 100 + [[1, 0, 1]] * 10)
        balancing = Balancing(strategy='weighting')
        init_params, fit_params = balancing.get_weights(
