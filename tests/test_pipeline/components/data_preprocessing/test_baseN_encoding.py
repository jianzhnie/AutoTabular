...
from autotabular.pipeline.components.data_preprocessing.category_encoders.baseN import BaseNEncoderTransformer
...
class BaseNEncoderTransformerTest(unittest.TestCase):
    ...
    def test_default_configuration(self):
        transformations = []
        for i in range(2):
            configuration_space = BaseNEncoderTransformer.get_hyperparameter_search_space(
            )
            default = configuration_space.get_default_configuration()

            preprocessor = BaseNEncoderTransformer(
                random_state=1,
                **{
