...
import autotabular.pipeline.base
import autotabular.pipeline.components.base
import autotabular.pipeline.components.classification
import autotabular.pipeline.components.feature_preprocessing
import ConfigSpace.configuration_space


class BasePipelineMock(autotabular.pipeline.base.BasePipeline):
...
class BaseTest(unittest.TestCase):
    ...
    def test_get_hyperparameter_configuration_space_3choices(self):
        ...
        pipeline = [
            ('p0',
             autotabular.pipeline.components.feature_preprocessing.
             FeaturePreprocessorChoice(dataset_properties)),
            ('p1',
             autotabular.pipeline.components.feature_preprocessing.
             FeaturePreprocessorChoice(dataset_properties)),
            ('c',
             autotabular.pipeline.components.classification.ClassifierChoice(
                 dataset_properties))
        ]

        base = BasePipelineMock()
        cs = base._get_base_search_space(cs, dataset_properties, exclude,
                                         include, pipeline)

        self.assertEqual(
            len(cs.get_hyperparameter('p0:__choice__').choices), 13)
        self.assertEqual(
            len(cs.get_hyperparameter('p1:__choice__').choices), 15)
        ...
        self.assertEqual(
            len(cs.get_hyperparameter('p0:__choice__').choices), 13)
        self.assertEqual(
            len(cs.get_hyperparameter('p1:__choice__').choices), 10)
        ...
        self.assertEqual(
            len(cs.get_hyperparameter('p1:__choice__').choices), 15)
        self.assertEqual(
            len(cs.get_hyperparameter('c:__choice__').choices), 16)
        # for clause in sorted([str(clause) for clause in cs.forbidden_clauses]):
        #    print(clause)
        ...
        cs = base._get_base_search_space(cs, dataset_properties, exclude,
                                         include, pipeline)
        self.assertEqual(
            len(cs.get_hyperparameter('p0:__choice__').choices), 12)
        self.assertEqual(
            len(cs.get_hyperparameter('p1:__choice__').choices), 15)
        # for clause in sorted([str(clause) for clause in cs.forbidden_clauses]):
        #    print(clause)
