...
from autotabular.constants import (BINARY_CLASSIFICATION, CLASSIFICATION_TASKS,
                                   ...
                                   REGRESSION_TASKS)
from autotabular.data.validation import SUPPORTED_FEAT_TYPES, SUPPORTED_TARGET_TYPES, InputValidator
from autotabular.data.xy_data_manager import XYDataManager
from autotabular.ensemble_builder import EnsembleBuilderManager
from autotabular.ensembles.singlebest_ensemble import SingleBest
from autotabular.evaluation import ExecuteTaFuncWithQueue, get_cost_of_crash
from autotabular.evaluation.abstract_evaluator import _fit_and_suppress_warnings
from autotabular.evaluation.train_evaluator import TrainEvaluator, _fit_with_budget
from autotabular.metrics import Scorer, accuracy, calculate_metric, f1_macro, r2
from autotabular.pipeline.base import BasePipeline
from autotabular.pipeline.components.classification import ClassifierChoice
...
from autotabular.pipeline.components.data_preprocessing.rescaling import RescalingChoice
from autotabular.pipeline.components.feature_preprocessing import FeaturePreprocessorChoice
from autotabular.pipeline.components.regression import RegressorChoice
from autotabular.smbo import AutoMLSMBO
from autotabular.util import RE_PATTERN, pipeline
from autotabular.util.backend import Backend
from autotabular.util.logging_ import get_named_client_logger, setup_logger, start_log_server
from autotabular.util.parallel import preload_modules
from autotabular.util.pipeline import parse_include_exclude_components
from autotabular.util.single_thread_client import SingleThreadedClient
from autotabular.util.stopwatch import StopWatch
...
def _model_predict(model, X, batch_size, logger, task):
    ...
    if len(prediction.shape) < 1 or len(X_.shape) < 1 or \
        ...
        logger.warning(
            'Prediction shape for model %s is %s while X_.shape is %s', model,
            str(prediction.shape), str(X_.shape))
    return prediction
...
class AutoML(BaseEstimator):
    ...
    def __init__(self,
        ...
        if isinstance(self._disable_evaluator_output, list):
            allowed_elements = [
                'model', 'cv_model', 'y_optimization', 'y_test', 'y_valid'
            ]
    ...
    def _close_dask_client(self):
        if (hasattr(self, '_is_dask_client_internally_created')
    ...
    def _clean_logger(self):
        if not hasattr(
                self,
                'stop_logging_server') or self.stop_logging_server is None:
            return
    ...
    def _do_dummy_prediction(self, datamanager: XYDataManager,
                             num_run: int) -> int:
    ...
    def fit(
        ...
        if not isinstance(self._metric, Scorer):
            raise ValueError('Metric must be instance of '
                             'autotabular.metrics.Scorer.')

        # If no dask client was provided, we create one, so that we can
        # start a ensemble process in parallel to smbo optimize
        if (self._dask_client is None
                and (self._ensemble_size > 0
                     or self._n_jobs is not None and self._n_jobs > 1)):
        ...
        try:
            self._logger.debug('  Distribution: %s',
        ...
        requirements = pkg_resources.resource_string('autotabular',
                                                     'requirements.txt')
        ...
        self._logger.debug('  time_left_for_this_task: %f',
                           self._time_for_task)
        self._logger.debug('  per_run_time_limit: %f',
        ...
        self._logger.debug('  max_models_on_disc: %s',
                           str(self._max_models_on_disc))
        self._logger.debug('  seed: %d', self._seed)
        self._logger.debug('  memory_limit: %s', str(self._memory_limit))
        self._logger.debug('  metadata_directory: %s',
                           self._metadata_directory)
        self._logger.debug('  debug_mode: %s', self._debug_mode)
        self._logger.debug('  include_estimators: %s',
                           str(self._include_estimators))
        self._logger.debug('  exclude_estimators: %s',
                           str(self._exclude_estimators))
        self._logger.debug('  include_preprocessors: %s',
                           str(self._include_preprocessors))
        self._logger.debug('  exclude_preprocessors: %s',
                           str(self._exclude_preprocessors))
        self._logger.debug('  resampling_strategy: %s',
                           str(self._resampling_strategy))
        ...
        self._logger.debug('  multiprocessing_context: %s',
                           str(self._multiprocessing_context))
        self._logger.debug('  dask_client: %s', str(self._dask_client))
        self._logger.debug('  precision: %s', str(self.precision))
        self._logger.debug('  disable_evaluator_output: %s',
                           str(self._disable_evaluator_output))
        self._logger.debug('  get_smac_objective_callback: %s',
                           str(self._get_smac_object_callback))
        self._logger.debug('  smac_scenario_args: %s',
        ...
        if proc_ensemble is not None:
            ...
            if len(self.ensemble_performance_history) > 0:
                pd.DataFrame(self.ensemble_performance_history).to_json(
                    os.path.join(self._backend.internals_directory,
    ...
    def subsample_if_too_large(
        ...
        if memory_limit and isinstance(X, np.ndarray):
            ...
            if memory_limit <= megabytes * 10:
                new_num_samples = int(
                    memory_limit /
                    (10 * X.shape[1] * multiplier / 1024 / 1024))
                logger.warning(
    ...
    def refit(self, X, y):
        ...
        if self.models_ is None or len(
                self.models_) == 0 or self.ensemble_ is None:
            self._load_models()
    ...
    def fit_pipeline(
        ...
        **kwargs: Dict,
        ...
        """Fits and individual pipeline configuration and returns the result to
        the user.
        ...
        if self.configuration_space is None:
            self.configuration_space = self.fit(
                ...
                y=y,
                dataset_name=dataset_name
                if dataset_name is not None else self._dataset_name,
                X_test=X_test,
                y_test=y_test,
                feat_type=feat_type,
        ...
        pipeline = None
        if kwargs['disable_file_output'] or kwargs[
                'resampling_strategy'] == 'test':
            self._logger.warning(
                'File output is disabled. No pipeline can returned')
    ...
    def predict(self, X, batch_size=None, n_jobs=1):
        ...
        if (self._resampling_strategy
                not in ('holdout', 'holdout-iterative-fit', 'cv',
                        'cv-iterative-fit') and not self._can_predict):
        ...
        all_predictions = joblib.Parallel(n_jobs=n_jobs)(
            joblib.delayed(_model_predict)(models[identifier], X, batch_size,
                                           self._logger, self._task)
    ...
    def fit_ensemble(self,
        ...
        manager = EnsembleBuilderManager(
            ...
            if ensemble_size else self._ensemble_size,
            ensemble_nbest=ensemble_nbest
            if ensemble_nbest else self._ensemble_nbest,
            max_models_on_disc=self._max_models_on_disc,
            seed=self._seed,
    ...
    def _load_models(self):
        ...
        if self.ensemble_:
            ...
            if self._resampling_strategy in ('cv', 'cv-iterative-fit'):
                ...
                self.cv_models_ = None
            if (len(self.models_) == 0 and self._resampling_strategy
                    not in ['partial-cv', 'partial-cv-iterative-fit']):
    ...
    def _load_best_individual_model(self):
        """In case of failure during ensemble building, this method returns the
        single best model found by AutoML.
    ...
    def score(self, X, y):
        ...
        # We train autotabular with a encoded version of y,
        ...
        for run_key in self.runhistory_.data:
            ...
            for metric in self._scoring_functions:
                if metric.name in run_value.additional_info.keys():
                    ...
                    metric_value = metric._optimum - (
                        metric._sign * metric_cost)
                    mask_value = False
                else:
                    metric_value = np.NaN
                    mask_value = True
    ...
    def _create_search_space(self,
        ...
        backend.write_txt_file(configspace_path,
