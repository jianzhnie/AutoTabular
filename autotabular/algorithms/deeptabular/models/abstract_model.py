...
from pytorch_tabnet.utils import ComplexEncoder, PredictDataset, create_dataloaders, define_device, validate_eval_set
...
class TabularModel(BaseEstimator):
    ...
    def fit(
        ...
        """Train a neural network stored in self.network Using train_dataloader
        for training data and valid_dataloader for validation.
    ...
    def save_model(self, path):
        ...
        # Save models params
        with open(
                Path(path).joinpath('model_params.json'), 'w',
                encoding='utf8') as f:
        ...
        if (self.early_stopping_metric is not None) and (self.patience > 0):
            early_stopping = EarlyStopping(
                ...
                is_maximize=(self._metrics[-1]._maximize
                             if len(self._metrics) > 0 else None),
    ...
    def _set_optimizer(self):
        """Setup optimizer."""
        self._optimizer = self.optimizer_fn(self.network.parameters(),
