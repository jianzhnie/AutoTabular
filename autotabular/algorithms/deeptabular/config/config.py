...
class DataConfig:
    ...
    date_columns: List = field(
        ...
        metadata={
            'help':
            "(Column names, Freq) tuples of the date fields. For eg. a field named introduction_date and with a monthly  \
            frequency should have an entry ('intro_date','M'}"
        },
    )
    ...
    encode_date_columns: bool = field(
        default=True,
        metadata={
            'help': 'Whether or not to encode the derived variables from date'
        },
    )
    ...
    continuous_feature_transform: Optional[str] = field(
        ...
        metadata={
            'help':
            'Whether or not to transform the features before modelling. By default it is turned off.',
            'choices': [
                None,
                'yeo-johnson',
                'box-cox',
    ...
    def __post_init__(self):
        assert (
            len(self.categorical_cols) + len(self.continuous_cols) +
            len(self.date_columns) > 0
        ...
        self.continuous_dim = (
            ...
            if self.continuous_cols is not None else 0)
...
class TrainerConfig:
    ...
    gpus: Optional[int] = field(
        ...
        metadata={
            'help':
            'Number of gpus to train on (int). -1 uses all available GPUs. By default uses CPU (None)'
        },
    )
    ...
    profiler: Optional[str] = field(
        ...
        metadata={
            'help':
            'To profile individual steps during training and assist in identifying bottlenecks. None, simple or advanced',
...
class ExperimentConfig:
    ...
    log_logits: bool = field(
        default=False,
        metadata={
            'help': 'Turn this on to log the logits as a histogram in W&B'
        },
    )
    ...
    exp_log_freq: int = field(
        default=100,
        metadata={
            'help': 'step count between logging of gradients and parameters.'
        },
    )
...
class OptimizerConfig:
    ...
    optimizer: str = field(
        ...
        metadata={
            'help':
            'Any of the standard optimizers from [torch.optim](https://pytorch.org/docs/stable/optim.html#algorithms).'
        },
    )
...
class ExperimentRunManager:
    ...
    def __init__(
        ...
        """The manages the versions of the experiments based on the name. It is
        a simple dictionary(yaml) based lookup. Primary purpose is to avoid
        overwriting of saved models while runing the training without changing
        the experiment name.
...
class ModelConfig:
    ...
    task: str = field(
        ...
        metadata={
            'help':
            'Specify whether the problem is regression of classification.',
    ...
    metrics_params: Optional[List] = field(
        default=None,
        metadata={
            'help': 'The parameters to be passed to the metrics function'
        },
    )
    ...
    def __post_init__(self):
        if self.task == 'regression':
            ...
            self.metrics_params = ([{} for _ in self.metrics]
                                   if self.metrics_params is None else
                                   self.metrics_params)
        else:
