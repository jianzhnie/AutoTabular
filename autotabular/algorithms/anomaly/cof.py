...
class COF(BaseDetector):
    ...
    distance of k nearest neighbor of the data point, as the outlier score for
    observations.
    ...
    def __init__(self, contamination=0.1, n_neighbors=20, method='fast'):
        ...
        if isinstance(n_neighbors, int):
            ...
            raise TypeError('n_neighbors should be int. Got %s' %
                            type(n_neighbors))
    ...
    def decision_function(self, X):
        ...
        if self.method.lower() == 'fast':
            ...
            raise ValueError(
                "method should be set to either \'fast\' or \'memory\'. Got %s"
                % self.method)

    def _cof_memory(self, X):
        """Connectivity-Based Outlier Factor (COF) Algorithm This function is
        called internally to calculate the Connectivity-Based Outlier Factor
        (COF) as an outlier score for observations.
