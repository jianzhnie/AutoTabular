...
import autotabular.metalearning
...
from autotabular.metalearning.metafeatures.metafeatures import calculate_all_metafeatures_encoded_labels, calculate_all_metafeatures_with_labels
from autotabular.metalearning.metalearning.meta_base import MetaBase
from autotabular.metalearning.mismbo import suggest_via_metalearning
...
def _calculate_metafeatures(data_feat_type, data_info_task, basename, x_train,
                            y_train, watcher, logger_):
...
class AutoMLSMBO(object):
    ...
    def collect_metalearning_suggestions(self, meta_base):
        metalearning_configurations = _get_metalearning_configurations(
            ...
            logger=self.logger)
        _print_debug_info_of_init_configuration(metalearning_configurations,
                                                self.dataset_name,
                                                self.total_walltime_limit,
                                                self.logger, self.watcher)
        ...
        if self.num_metalearning_cfgs > 0:
            ...
            if self.metadata_directory is None:
                metalearning_directory = os.path.dirname(
                    autotabular.metalearning.__file__)
            ...
            # then verify that it exists.
                ...
                if not os.path.exists(self.metadata_directory):
                    ...
                    metadata_directory = os.path.join(
                        self.metadata_directory, '%s_%s_%s' %
                        (self.metric, TASK_TYPES_TO_STRING[meta_task], 'sparse'
                         if self.datamanager.info['is_sparse'] else 'dense'))
            ...
            if os.path.exists(self.metadata_directory):
                ...
                meta_base = MetaBase(self.config_space,
                                     self.metadata_directory, self.logger)
                ...
                if metafeature_calculation_time_limit < 1:
                    ...
                    with warnings.catch_warnings():
                        warnings.showwarning = get_send_warnings_to_logger(
                            self.logger)
                ...
                if meta_features is not None:
                    ...
                    all_metafeatures.fillna(
                        all_metafeatures.mean(), inplace=True)
