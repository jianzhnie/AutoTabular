...
from autotabular.constants import REGRESSION_TASKS, TASK_TYPES
...
class Scorer(object, metaclass=ABCMeta):
    ...
    def __init__(self, name: str, score_func: Callable, optimum: float,
                 worst_possible_result: float, sign: float,
                 kwargs: Any) -> None:
        self.name = name
        self._kwargs = kwargs
    ...
    def __call__(self,
                 y_true: np.ndarray,
                 y_pred: np.ndarray,
                 sample_weight: Optional[List[float]] = None) -> float:
        pass
...
class _ProbaScorer(Scorer):
    ...
    def __call__(self,
        ...
        if self._score_func is sklearn.metrics.log_loss:
            n_labels_pred = np.array(y_pred).reshape(
                (len(y_pred), -1)).shape[1]
...
def make_scorer(name: str,
    ...
    **kwargs : additional arguments
    ...
    if needs_proba:
        return _ProbaScorer(name, score_func, optimum, worst_possible_result,
                            sign, kwargs)
    elif needs_threshold:
        return _ThresholdScorer(name, score_func, optimum,
                                worst_possible_result, sign, kwargs)
    else:
        return _PredictScorer(name, score_func, optimum, worst_possible_result,
                              sign, kwargs)
