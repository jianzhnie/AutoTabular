...
class MetaFeatureValue(object):
    ...
    def to_arff_row(self):
        ...
        return [
            self.name, self.type_, self.fold, self.repeat, value, self.time,
            self.comment
        ]
...
class DatasetMetafeatures(object):
    ...
    def _get_arff(self):
        ...
        output['description'] = ''
        output['attributes'] = [('name', 'STRING'), ('type', 'STRING'),
                                ('fold', 'NUMERIC'), ('repeat', 'NUMERIC'),
                                ('value', 'NUMERIC'), ('time', 'NUMERIC'),
                                ('comment', 'STRING')]
        output['data'] = []
    ...
    def __repr__(self, verbosity=0):
        ...
        for name in self.metafeature_values:
            if verbosity == 0 and self.metafeature_values[
                    name].type_ != 'METAFEATURE':
                continue
            if verbosity == 0:
                repr.write(
                    '  %s: %s\n' %
