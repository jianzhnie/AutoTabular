...
class HelperFunctions(object):
    ...
    def is_calculated(self, key):
        ...
        Necessary as get_value() can return None if the helper function hasn't been executed or if it returned None.
        """
    ...
    def define(self, name):
        ...
        This behaves like a function decorating a function, not a class decorating a function
        """
...
class NumberOfClasses(MetaFeature):
    ...
    Calls np.unique on the targets. If the dataset is a multilabel dataset, does this for each label seperately and returns the mean.
    """
...
class MissingValues(HelperFunction):
    ...
    def _calculate_sparse(self, X, y, logger, categorical):
        ...
        missing = X.__class__((data, X.indices, X.indptr),
                              shape=X.shape,
                              dtype=np.bool)
        return missing
...
@metafeatures.define(
    'NumberOfInstancesWithMissingValues', dependency='MissingValues')
class NumberOfInstancesWithMissingValues(MetaFeature):
    ...
    def _calculate_sparse(self, X, y, logger, categorical):
        ...
        num_missing = [
            np.sum(
                new_missing.data[new_missing.indptr[i]:new_missing.indptr[i +
                                                                          1]])
...
@metafeatures.define(
    'NumberOfFeaturesWithMissingValues', dependency='MissingValues')
...
class NumberOfMissingValues(MetaFeature):
    ...
    def _calculate(self, X, y, logger, categorical):
        if scipy.sparse.issparse(X):
            ...
            return float(
                np.count_nonzero(helper_functions.get_value('MissingValues')))


@metafeatures.define(
    'PercentageOfMissingValues', dependency='NumberOfMissingValues')
...
class RatioNumericalToNominal(MetaFeature):
    ...
    def _calculate(self, X, y, logger, categorical):
        num_categorical = float(metafeatures['NumberOfCategoricalFeatures'](
            X, y, logger, categorical).value)
        num_numerical = float(metafeatures['NumberOfNumericFeatures'](
            X, y, logger, categorical).value)
...
@metafeatures.define(
    'LogInverseDatasetRatio', dependency='InverseDatasetRatio')
    ...
    def _calculate(self, X, y, logger, categorical):
        if len(y.shape) == 2:
            ...
            for i in range(y.shape[1]):
                occurences.append(
                    self._calculate(X, y[:, i], logger, categorical))
            return occurences
        else:
...
class SymbolsMean(MetaFeature):
    ...
    def _calculate(self, X, y, logger, categorical):
        ...
        # measure
        values = [
            val for val in helper_functions.get_value('NumSymbols') if val > 0
        ]
...
@metafeatures.define(
    'PCAFractionOfComponentsFor95PercentVariance', dependency='PCA')
...
def calculate_all_metafeatures_encoded_labels(X,
                                              y,
                                              categorical,
                                              dataset_name,
                                              logger,
...
def calculate_all_metafeatures_with_labels(X,
                                           y,
                                           categorical,
                                           dataset_name,
                                           logger,
...
npy_metafeatures = set([
    'LandmarkLDA', 'LandmarkNaiveBayes', 'LandmarkDecisionTree',
    ...
    'PCASkewnessFirstPC', 'Skewnesses', 'SkewnessMin', 'SkewnessMax',
    'SkewnessMean', 'SkewnessSTD', 'Kurtosisses', 'KurtosisMin', 'KurtosisMax',
...
subsets['pfahringer_2000_experiment1'] = set([
    'number_of_features', 'number_of_numeric_features',
...
subsets['bardenet_2013_boost'] = set([
    'number_of_classes', 'log_number_of_features', 'log_inverse_dataset_ratio',
    'pca_95percent'
])
