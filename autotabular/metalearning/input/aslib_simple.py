...
class AlgorithmSelectionProblem(object):
    ...
    def _find_files(self):
        ...
        for expected_file in expected:
            ...
            if not os.path.isfile(full_path):
                self.logger.error('Not found: %s (has to be added)' %
                                  (full_path))
            else:
    ...
    def _read_files(self):
        """iterates over all found files (self.found_files) and calls the
        corresponding function to validate file."""
    ...
    def _read_algorithm_runs(self, filename):
        ...
        if arff_dict['attributes'][2][0].upper() != 'ALGORITHM':
            self.logger.error('algorithm as third attribute is missing in %s' %
                              (filename))

        performance_measures = [pm[0] for pm in arff_dict['attributes'][3:-1]]

        measure_instance_algorithm_triples = defaultdict(
    ...
    def _read_feature_values(self, filename):
        ...
        for data in arff_dict['data']:
            ...
            metafeatures[inst_name] = {
                ...
                for feature, feature_value in zip(arff_dict['attributes'][2:],
                                                  features)
            }
