...
import autotabular.pipeline.create_searchspace_util
import numpy as np
import scipy.sparse
...
from .components.base import AutotabularChoice, AutotabularComponent

DATASET_PROPERTIES_TYPE = Dict[str, Union[str, int, bool]]
PIPELINE_DATA_DTYPE = Union[np.ndarray, scipy.sparse.bsr_matrix,
                            scipy.sparse.coo_matrix, scipy.sparse.csc_matrix,
                            scipy.sparse.csr_matrix, scipy.sparse.dia_matrix,
                            scipy.sparse.dok_matrix, scipy.sparse.lil_matrix, ]
...
class BasePipeline(Pipeline):
    ...
    def set_hyperparameters(self, configuration, init_params=None):
        ...
        for node_idx, n_ in enumerate(self.steps):
            ...
            if isinstance(
                    node,
                (AutotabularChoice, AutotabularComponent, BasePipeline)):
    ...
    def _get_base_search_space(self, cs, dataset_properties, exclude, include,
                               pipeline):
        if include is None:
        ...
        matches = autotabular.pipeline.create_searchspace_util.get_match_array(
        ...
        for node_idx, n_ in enumerate(pipeline):
            ...
            is_choice = isinstance(node, AutotabularChoice)
            ...
            #  choices are actually legal choices
                ...
                choices_list = autotabular.pipeline.create_searchspace_util.\
        ...
        if np.sum(matches) < np.size(matches):
            cs = autotabular.pipeline.create_searchspace_util.add_forbidden(
                conf_space=cs,
                pipeline=pipeline,
                matches=matches,
                dataset_properties=dataset_properties,
    ...
    def _check_init_params_honored(self, init_params):
        """Makes sure that init params is honored at the implementation
        level."""
        ...
        for key, value in init_params.items():
            ...
            if isinstance(node, AutotabularComponent):
                node_dict = vars(node)
            elif isinstance(node, AutotabularChoice):
                node_dict = vars(node.choice)
            else:
                raise ValueError('Unsupported node type {}'.format(type(node)))

            if variable_name not in node_dict or node_dict[
                    variable_name] != value:
                raise ValueError(
        ...
        if len(self.dataset_properties) > 0:
            ...
            for i, item in enumerate(self.dataset_properties.items()):
                ...
                if isinstance(item[1], str):
                    dataset_properties_string.append("'%s': '%s'" %
                                                     (item[0], item[1]))
                else:
                    dataset_properties_string.append("'%s': %s" %
                                                     (item[0], item[1]))
    ...
    def get_additional_run_info(self):
        ...
        Can be overridden by subclasses to return additional information to the optimization algorithm.
        """
