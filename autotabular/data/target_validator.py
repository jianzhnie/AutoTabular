...
SUPPORTED_TARGET_TYPES = typing.Union[typing.List, pd.Series, pd.DataFrame,
                                      np.ndarray, scipy.sparse.bsr_matrix,
            ...
            if isinstance(y_train, pd.DataFrame):
                ...
                if y_train.columns.tolist() != y_test.columns.tolist():
                    raise ValueError(
                        ...
                        'y={} and y_test={} '.format(y_train.columns,
                                                     y_test.columns))
        ...
        if self.encoder is not None:
            ...
            if len(shape) > 1:
                ...
                if hasattr(y, 'iloc'):
                    ...
                    y = self.encoder.transform(y.to_numpy().reshape(
                        -1, 1)).reshape(-1)
                else:
                    y = self.encoder.transform(np.array(y).reshape(
                        -1, 1)).reshape(-1)
