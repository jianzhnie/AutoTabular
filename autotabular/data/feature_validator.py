...
import pandas as pd
import scipy.sparse
from autotabular.util.logging_ import PickableLoggerAdapter
from pandas.api.types import is_numeric_dtype, is_sparse
from sklearn.base import BaseEstimator
from sklearn.exceptions import NotFittedError

SUPPORTED_FEAT_TYPES = typing.Union[typing.List, pd.DataFrame, np.ndarray,
...
class FeatureValidator(BaseEstimator):
    ...
    def __init__(
        ...
        self.feat_type: typing.Optional[typing.Dict[typing.Union[str, int],
                                                    str]] = None
    ...
    def fit(
        ...
        if hasattr(X_train, 'iloc'):
            ...
            if self.feat_type is None:
                ...
                if not all(
                    [isinstance(f, str) for f in self.feat_type.values()]):
                    raise ValueError(
    ...
    def get_feat_type_from_columns(
        ...
        """Returns a dictionary that maps pandas dataframe columns to a feature
        type. This feature type can be categorical or numerical.
    ...
    def list_to_dataframe(
        ...
        """Converts a list to a pandas DataFrame. In this process, column types
        are inferred.
        ...
        if len(self.dtypes) == 0:
            ...
            self.dtypes = {
                col: X_train[col].dtype.name.lower()
                for col in X_train.columns
            }
        else:
            for col in X_train.columns:
        ...
        self.logger.warning(
            'The provided feature types to autotabular are of type list.'
