...
import pandas as pd
import pynisher
from autotabular.constants import BINARY_CLASSIFICATION
from autotabular.ensembles.abstract_ensemble import AbstractEnsemble
from autotabular.ensembles.ensemble_selection import EnsembleSelection
from autotabular.metrics import Scorer, calculate_loss, calculate_score
from autotabular.util.backend import Backend
from autotabular.util.logging_ import get_named_client_logger
...
class EnsembleBuilderManager(IncorporateRunResultCallback):
    ...
    def __init__(
        ...
        """SMAC callback to handle ensemble building.
            ...
            ensemble_size: int
                maximal size of ensemble (passed to autotabular.ensemble.ensemble_selection)
    ...
    def build_ensemble(self,
        ...
        if self.time_left_for_ensembles < elapsed_time:
            logger.info(
                'Terminate ensemble building as not time is left (run for {}s)'
                .format(elapsed_time), )
            return
        if self.max_iterations is not None and self.max_iterations <= self.iteration:
            logger.info(
                'Terminate ensemble building because of max iterations: {} of {}'
                .format(self.max_iterations, self.iteration))
            return
        ...
        if len(self.futures) == 0:
            ...
            try:
                ...
                logger.info(
                    '{}/{} Started Ensemble builder job at {} for iteration {}.'
                    .format(
...
) -> Tuple[List[Tuple[int, float, float, float]], int, Optional[np.ndarray],
           Optional[np.ndarray], Optional[np.ndarray], ]:
    """A short function to fit and create an ensemble. It is just a wrapper to
    easily send a request to dask to create an ensemble and clean the memory
    when finished.
...
class EnsembleBuilder(object):
        ...
        if os.path.exists(self.ensemble_memory_file):
            try:
                ...
                self.logger.warning(
                    ...
                    'This might impact the quality of the run. Exception={} {}'
                    .format(
                        e,
    ...
    def run(
        ...
        while True:
            ...
            if safe_ensemble_script.exit_status is pynisher.MemorylimitException:
                ...
                if isinstance(self.ensemble_nbest,
                              numbers.Integral) and self.ensemble_nbest <= 1:
                    if self.read_at_most == 1:
                        self.logger.error(
                            ...
                            'loaded per iteration -- please restart Auto-tabular with a higher '
                    ...
                    if isinstance(self.ensemble_nbest, numbers.Integral):
                        self.ensemble_nbest = max(1,
                                                  int(self.ensemble_nbest / 2))
                    else:
    ...
    def main(self, time_left, iteration, return_predictions):
        ...
        if candidate_models_set.intersection(n_sel_valid).intersection(
                n_sel_test):
            candidate_models = sorted(
                list(
                    candidate_models_set.intersection(
        ...
        # Save the ensemble for later use in the main Auto-tabular module!
    ...
    def get_disk_consumption(self, pred_path):
        ...
        stored_files_for_run = [
            os.path.join(
                self.backend.get_numrun_directory(_seed, _num_run, _budget),
                file_name) for file_name in stored_files_for_run
        ]
        this_model_cost = sum(
            [os.path.getsize(path) for path in stored_files_for_run])
    ...
    def compute_loss_per_model(self):
        ...
        for y_ens_fn, match, _seed, _num_run, _budget, mtime in \
            ...
            try:
                ...
                self.read_losses[y_ens_fn]['mtime_ens'] = os.path.getmtime(
                    y_ens_fn)
    ...
    def get_n_best_preds(self):
        ...
        if not sorted_keys:
            ...
            if num_keys > num_dummy:
                self.logger.warning(
                    ...
                    'Number of dummy models: %d', num_keys - 1, num_dummy)
            sorted_keys = [(k, v['ens_loss'], v['num_run'])
        ...
        if not isinstance(self.ensemble_nbest, numbers.Integral):
            ...
            self.logger.debug(
                ...
                '(%d out of %d)', self.ensemble_nbest * 100, keep_nbest,
                len(sorted_keys))
        else:
        ...
        if self.max_models_on_disc is not None:
            if not isinstance(self.max_models_on_disc, numbers.Integral):
                ...
                if (sum(c[1] for c in consumption) +
                        max_consumption) > self.max_models_on_disc:
                    ...
                    sorted_cum_consumption = np.cumsum(
                        [c[1]
                         for c in list(sorted(consumption))]) + max_consumption
                    ...
                    self.logger.warning(
                        ...
                        ' as accumulated={} worst={} num_models={}'.format(
                            ...
                            max_consumption, self.max_resident_models))
                else:
                    self.max_resident_models = None
            else:
        ...
        if self.performance_range_threshold > 0:
            ...
            worst_loss -= (worst_loss -
                           best_loss) * self.performance_range_threshold
                ...
                for i in range(0, keep_nbest):
                    ...
                    if current_loss >= worst_loss:
                        self.logger.debug(
                            ...
                            'Further reduce from %d to %d models', keep_nbest,
                            max(1, i))
                        keep_nbest = max(1, i)
                        break
        ...
        for k in sorted_keys[:ensemble_n_best]:
            if ((k not in self.read_preds
                 or self.read_preds[k][Y_ENSEMBLE] is None)
    ...
    def get_valid_test_preds(
            self, selected_keys: List[str]) -> Tuple[List[str], List[str]]:
        """get valid and test predictions from disc and store them in
        self.read_preds.
        ...
        for k in selected_keys:
            ...
            if len(valid_fn) == 0:
                ...
                valid_fn = valid_fn[0]
                if (self.read_losses[k]['mtime_valid']
                        == os.path.getmtime(valid_fn) and k in self.read_preds
                ...
                try:
                    ...
                    success_keys_valid.append(k)
                    self.read_losses[k]['mtime_valid'] = os.path.getmtime(
                        valid_fn)
                except Exception:
            ...
            if len(test_fn) == 0:
                ...
                test_fn = test_fn[0]
                if (self.read_losses[k]['mtime_test']
                        == os.path.getmtime(test_fn) and k in self.read_preds
                ...
                try:
                    ...
                    success_keys_test.append(k)
                    self.read_losses[k]['mtime_test'] = os.path.getmtime(
                        test_fn)
                except Exception:
    ...
    def predict(self, set_: str, ensemble: AbstractEnsemble,
                selected_keys: list, n_preds: int, index_run: int):
    ...
    def _add_ensemble_trajectory(self, train_pred, valid_pred, test_pred):
        ...
        if self.task_type == BINARY_CLASSIFICATION:
            if len(train_pred.shape) == 1 or train_pred.shape[1] == 1:
                train_pred = np.vstack(((1 - train_pred).reshape(
                    (1, -1)), train_pred.reshape((1, -1)))).transpose()
            if valid_pred is not None and (len(valid_pred.shape) == 1
                                           or valid_pred.shape[1] == 1):
                valid_pred = np.vstack(((1 - valid_pred).reshape(
                    (1, -1)), valid_pred.reshape((1, -1)))).transpose()
            if test_pred is not None and (len(test_pred.shape) == 1
                                          or test_pred.shape[1] == 1):
                test_pred = np.vstack(((1 - test_pred).reshape(
                    (1, -1)), test_pred.reshape((1, -1)))).transpose()
    ...
    def _get_list_of_sorted_preds(self):
        """Returns a list of sorted predictions in descending order Losses are
        taken from self.read_losses.
    ...
    def _delete_excess_models(self, selected_keys: List[str]):
        """Deletes models excess models on disc.

        self.max_models_on_disc defines the upper limit on how many models to keep. Any additional model with a worst loss than the top self.max_models_on_disc
        is deleted.
        """
        ...
        for pred_path in self.y_ens_files:
            ...
            numrun_dir = self.backend.get_numrun_directory(
                _seed, _num_run, _budget)
            try:
                os.rename(numrun_dir, numrun_dir + '.old')
                shutil.rmtree(numrun_dir + '.old')
                self.logger.info('Deleted files of non-candidate model %s',
                                 pred_path)
    ...
    def _read_np_fn(self, path):
        ...
        if isinstance(self.precision, str):
            ...
            self.logger.warning(
                'Interpreted str-precision as {}'.format(precision))
        else:
            precision = self.precision
        ...
        with open_method(path, 'rb') as fp:
            if precision == 16:
                predictions = np.load(
                    fp, allow_pickle=True).astype(dtype=np.float16)
            elif precision == 32:
                predictions = np.load(
                    fp, allow_pickle=True).astype(dtype=np.float32)
            elif precision == 64:
                predictions = np.load(
                    fp, allow_pickle=True).astype(dtype=np.float64)
            else:
