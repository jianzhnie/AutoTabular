...
class AvazuDataset(torch.utils.data.Dataset):
    ...
    def __init__(self,
        ...
        self.env = lmdb.open(
            cache_path, create=False, lock=False, readonly=True)
    ...
    def __getitem__(self, index):
        with self.env.begin(write=False) as txn:
            np_array = np.frombuffer(
                ...
                dtype=np.uint32).astype(dtype=np.long)
    ...
    def __get_feat_mapper(self, path):
        ...
        feat_mapper = {
            i: {feat
                for feat, c in cnt.items() if c >= self.min_threshold}
    ...
    def __yield_buffer(self, path, feat_mapper, defaults,
        ...
        with open(path) as f:
            ...
            for line in pbar:
                ...
                for i in range(1, self.NUM_FEATS + 1):
                    np_array[i] = feat_mapper[i].get(values[i + 1],
                                                     defaults[i])
                buffer.append((struct.pack('>I',
                                           item_idx), np_array.tobytes()))
                item_idx += 1
