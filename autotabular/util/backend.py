...
def get_randomized_directory_name(
        temporary_directory: Optional[str] = None) -> str:
    uuid_str = str(uuid.uuid1(clock_seq=os.getpid()))

    temporary_directory = (
        temporary_directory if temporary_directory else os.path.join(
            tempfile.gettempdir(),
            'autotabular_tmp_{}'.format(uuid_str, ),
        ))
...
class BackendContext(object):
    ...
    def __init__(
        ...
        # attributes to check that directories were created by autotabular.
    ...
    def temporary_directory(self) -> str:
        ...
        return os.path.expanduser(
            os.path.expandvars(self._temporary_directory))
    ...
    * true targets of the ensemble
    ...
    def save_start_time(self, seed: str) -> str:
        ...
        with tempfile.NamedTemporaryFile(
                'w', dir=os.path.dirname(filepath), delete=False) as fh:
    ...
    def save_targets_ensemble(self, targets: np.ndarray) -> str:
        ...
        with tempfile.NamedTemporaryFile(
                'wb', dir=os.path.dirname(filepath), delete=False) as fh_w:
    ...
    def save_datamanager(self, datamanager: AbstractDataManager) -> str:
        ...
        with tempfile.NamedTemporaryFile(
                'wb', dir=os.path.dirname(filepath), delete=False) as fh:
    ...
    def get_numrun_directory(self, seed: int, num_run: int,
                             budget: float) -> str:
        return os.path.join(self.internals_directory, 'runs',
                            '%d_%d_%s' % (seed, num_run, budget))
    ...
    def list_all_models(self, seed: int) -> List[str]:
        ...
        model_files = glob.glob(
            os.path.join(
                glob.escape(runs_directory), '%d_*' % seed,
                '%s.*.*.model' % seed))
        return model_files

    def load_models_by_identifiers(
            self, identifiers: List[Tuple[int, int, float]]) -> Dict:
        models = dict()
    ...
    def load_model_by_seed_and_id_and_budget(self, seed: int, idx: int,
                                             budget: float) -> Pipeline:
    ...
    def load_cv_model_by_seed_and_id_and_budget(self, seed: int, idx: int,
                                                budget: float) -> Pipeline:
        model_directory = self.get_numrun_directory(seed, idx, budget)
    ...
    def save_numrun_to_dir(
        ...
        if model is not None:
            file_path = os.path.join(
                tmpdir, self.get_model_filename(seed, idx, budget))
        ...
        for preds, subset in ((ensemble_predictions, 'ensemble'),
                              (valid_predictions, 'valid'), (test_predictions,
                                                             'test')):
    ...
    def load_ensemble(self, seed: int) -> Optional[AbstractEnsemble]:
        ...
        if not os.path.exists(ensemble_dir):
            if self.logger is not None:
                self.logger.warning('Directory %s does not exist' %
                                    ensemble_dir)
            else:
        ...
        if seed >= 0:
            indices_files = glob.glob(
                os.path.join(
                    glob.escape(ensemble_dir), '%s.*.ensemble' % seed))
            indices_files.sort()
        else:
            indices_files = os.listdir(ensemble_dir)
            indices_files = [
                os.path.join(ensemble_dir, f) for f in indices_files
            ]
    ...
    def save_ensemble(self, ensemble: AbstractEnsemble, idx: int,
                      seed: int) -> None:
        try:
    ...
    def get_prediction_filename(self, subset: str, automl_seed: Union[str,
                                                                      int],
                                idx: int, budget: float) -> str:
        return 'predictions_%s_%s_%s_%s.npy' % (subset, automl_seed, idx,
                                                budget)
