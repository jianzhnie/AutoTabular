...
import numpy as np
from autotabular.constants import CLASSIFICATION_TASKS, MULTILABEL_CLASSIFICATION, MULTIOUTPUT_REGRESSION, REGRESSION_TASKS
from autotabular.data.abstract_data_manager import AbstractDataManager
from autotabular.data.validation import SUPPORTED_FEAT_TYPES, SUPPORTED_TARGET_TYPES
from autotabular.evaluation.abstract_evaluator import TYPE_ADDITIONAL_INFO, AbstractEvaluator, _fit_and_suppress_warnings
from autotabular.metrics import Scorer
from autotabular.pipeline.base import PIPELINE_DATA_DTYPE
from autotabular.pipeline.components.base import IterativeComponent
from autotabular.util.backend import Backend
from autotabular.util.logging_ import PicklableClientLogger
from ConfigSpace import Configuration
from sklearn.base import BaseEstimator
from sklearn.model_selection import BaseCrossValidator, KFold, PredefinedSplit, ShuffleSplit, StratifiedKFold, StratifiedShuffleSplit, train_test_split
from sklearn.model_selection._split import BaseShuffleSplit, _RepeatedSplits
from smac.tae import StatusType, TAEAbortException

__all__ = [
    'TrainEvaluator', 'eval_holdout', 'eval_iterative_holdout', 'eval_cv',
    'eval_partial_cv', 'eval_partial_cv_iterative'
]


def _get_y_array(y: SUPPORTED_TARGET_TYPES,
                 task_type: int) -> SUPPORTED_TARGET_TYPES:
...
def subsample_indices(train_indices: List[int], subsample: Optional[float],
                      task_type: int,
                      Y_train: SUPPORTED_TARGET_TYPES) -> List[int]:

    if not isinstance(subsample, float):
        raise ValueError('Subsample must be of type float, but is of type %s' %
                         type(subsample))
    elif subsample > 1:
        raise ValueError('Subsample must not be larger than 1, but is %f' %
                         subsample)
    ...
    if subsample is not None and subsample < 1:
        ...
        if task_type in CLASSIFICATION_TASKS and task_type != MULTILABEL_CLASSIFICATION:
            stratify: Optional[SUPPORTED_TARGET_TYPES] = Y_train.iloc[
                train_indices] if hasattr(Y_train,
                                          'iloc') else Y_train[train_indices]
        else:
            stratify = None
...
def _fit_with_budget(
    ...
    if (budget_type == 'iterations' or budget_type == 'mixed'
        ...
        if model.estimator_supports_iterative_fit():
            ...
            Xt, fit_params = model.fit_transformer(
                ...
                if hasattr(X_train, 'iloc') else X_train[train_indices],
                Y_train.iloc[train_indices]
                if hasattr(Y_train, 'iloc') else Y_train[train_indices],
            )
    ...
    elif (budget_type == 'subsample' or budget_type == 'mixed'
...
class TrainEvaluator(AbstractEvaluator):
    ...
    def __init__(
        ...
        resampling_strategy: Optional[Union[str, BaseCrossValidator,
                                            _RepeatedSplits,
                                            BaseShuffleSplit]] = None,
        resampling_strategy_args: Optional[Dict[str,
        ...
        self.indices: List[Optional[Tuple[List[int],
                                          List[int]]]] = [None
                                                          ] * self.num_cv_folds
    ...
    def fit_predict_and_loss(self, iterative: bool = False) -> None:
        ...
        if iterative:
            if self.num_cv_folds == 1:
                ...
                self.models = [
                    self._get_model() for i in range(self.num_cv_folds)
                ]
                ...
                if self.budget_type in ['iterations', 'mixed'
                ...
                while not all(converged):
                    ...
                    for i, (train_indices, test_indices) in enumerate(
                            splitter.split(
                                self.X_train,
                                y,
                                groups=self.resampling_strategy_args.get(
                                    'groups'))):
                        if converged[i]:
                            continue
                        ...
                        if iterations[i] == 1:
                            ...
                            Xt, fit_params = model.fit_transformer(
                                self.X_train.iloc[train_indices] if hasattr(
                                    self.X_train,
                                    'iloc') else self.X_train[train_indices],
                                self.Y_train.iloc[train_indices] if hasattr(
                                    self.Y_train, 'iloc') else
                                self.Y_train[train_indices],
                            )
                            Xt_array[i] = Xt
                            fit_params_array[i] = fit_params
                        n_iter = int(2**iterations[i] /
                                     2) if iterations[i] > 1 else 2
                        ...
                        (train_pred, opt_pred, valid_pred,
                         test_pred) = self._predict(
                             model,
                    ...
                    if self.X_valid is not None:
                        Y_valid_preds = np.array([
                            Y_valid_pred[i] for i in range(self.num_cv_folds)
                    ...
                    if self.X_test is not None:
                        Y_test_preds = np.array([
                            Y_test_pred[i] for i in range(self.num_cv_folds)
            ...
            for i, (train_split, test_split) in enumerate(
                ...
                if (additional_run_info is not None
                        and len(additional_run_info) > 0 and i > 0):
            ...
            if all(isinstance(elem, dict) for elem in train_losses):
                ...
                train_loss = np.average(
                    train_losses, weights=train_fold_weights)
            ...
            if self.num_cv_folds > 1:
                ...
                status = StatusType.SUCCESS
            elif (self.budget_type == 'iterations'
                ...
                n_iter = int(
                    np.ceil(cast(float, self.budget) / 100 * budget_factor))
    ...
    def partial_fit_predict_and_loss(self,
                                     fold: int,
                                     iterative: bool = False) -> None:
        """Fit, predict and compute the loss for eval_partial_cv (both
        iterative and normal)"""

        if fold > self.num_cv_folds:
            raise ValueError('Cannot evaluate a fold %d which is higher than '
                             'the number of folds %d.' %
                             (fold, self.num_cv_folds))
    ...
    def _partial_fit_and_predict_iterative(self, fold: int,
                                           train_indices: List[int],
        ...
        if model.estimator_supports_iterative_fit():
            ...
            self.Y_train_targets[
                train_indices] = self.Y_train.iloc[train_indices] if hasattr(
                    self.Y_train, 'iloc') else self.Y_train[train_indices]
            ...
            if self.budget is not None and self.budget > 0:
                max_n_iter_budget = int(
                    np.ceil(self.budget / 100 * model_max_iter))
            ...
            while (not model.configuration_fully_fitted()
                   and model_current_iter < max_iter):
                ...
                model.iterative_fit(
                    ...
                    n_iter=n_iter,
                    **fit_params)
                (Y_train_pred, Y_optimization_pred, Y_valid_pred,
                ...
                if model.configuration_fully_fitted(
                ) or model_current_iter >= max_iter:
                    final_call = True
                else:
                    final_call = False
            ...
            (Y_train_pred, Y_optimization_pred, Y_valid_pred, Y_test_pred,
             additional_run_info) = self._partial_fit_and_predict_standard(
    ...
    def _partial_fit_and_predict_standard(
        ...
        _fit_and_suppress_warnings(
            ...
            if hasattr(self.X_train, 'iloc') else self.X_train[train_indices],
            self.Y_train.iloc[train_indices]
            if hasattr(self.Y_train, 'iloc') else self.Y_train[train_indices],
        )
    ...
    def _predict(
        self, model: BaseEstimator, test_indices: List[int],
        train_indices: List[int]
        ...
        train_pred = self.predict_function(
            self.X_train.iloc[train_indices]
            if hasattr(self.X_train, 'iloc') else self.X_train[train_indices],
            model, self.task_type, self.Y_train.iloc[train_indices] if hasattr(
                self.Y_train, 'iloc') else self.Y_train[train_indices])

        opt_pred = self.predict_function(
            self.X_train.iloc[test_indices]
            if hasattr(self.X_train, 'iloc') else self.X_train[test_indices],
            model, self.task_type, self.Y_train.iloc[train_indices] if hasattr(
                self.Y_train, 'iloc') else self.Y_train[train_indices])
    ...
    def get_splitter(
        ...
        if self.resampling_strategy_args:
            train_size_from_user = self.resampling_strategy_args.get(
                'train_size')
            if train_size_from_user is not None:
                train_size = float(train_size_from_user)
        test_size = float('%.4f' % (1 - train_size))

        if D.info['task'] in CLASSIFICATION_TASKS and D.info[
            ...
            if self.resampling_strategy in [
                ...
                if shuffle:
                    try:
                        ...
                        if 'The least populated class in y has only' in e.args[
                                0]:
                            cv = ShuffleSplit(
                                n_splits=1,
                                test_size=test_size,
            ...
            if self.resampling_strategy in [
                ...
                if shuffle:
                    ...
                    'cv', 'partial-cv', 'partial-cv-iterative-fit'
            ]:
    ...
    def check_splitter_resampling_strategy(
        ...
        if (task in CLASSIFICATION_TASKS and task != MULTILABEL_CLASSIFICATION
                or
            (task in REGRESSION_TASKS and task != MULTIOUTPUT_REGRESSION)):
            y = y.ravel()
...
def eval_holdout(
    ...
    backend: Backend,
    resampling_strategy: Union[str, BaseCrossValidator, _RepeatedSplits,
                               BaseShuffleSplit],
...
def eval_partial_cv(
    ...
    if budget_type is not None:
        raise NotImplementedError()
    instance_dict: Dict[str, int] = json.loads(
