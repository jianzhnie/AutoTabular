...
import autotabular.evaluation.test_evaluator
import autotabular.evaluation.train_evaluator
import autotabular.evaluation.util
import numpy as np
import pynisher
from autotabular.evaluation.train_evaluator import TYPE_ADDITIONAL_INFO
from autotabular.metrics import Scorer
from autotabular.util.backend import Backend
from autotabular.util.logging_ import PickableLoggerAdapter, get_named_client_logger
...
def fit_predict_try_except_decorator(ta: Callable,
    ...
    try:
        ...
        # following traceback:
        #     File "auto-sklearn/autotabular/evaluation/__init__.py", line 29, in fit_predict_try_except_decorator  # noqa E501
        #     return ta(queue=queue, **kwargs)
        #     File "auto-sklearn/autotabular/evaluation/train_evaluator.py", line 1067, in eval_holdout  # noqa E501
        #     evaluator.fit_predict_and_loss(iterative=iterative)
        #     File "auto-sklearn/autotabular/evaluation/train_evaluator.py", line 616, in fit_predict_and_loss,  # noqa E501
        #     status=status
        #     File "auto-sklearn/autotabular/evaluation/abstract_evaluator.py", line 320, in finish_up  # noqa E501
        #     self.queue.put(rval_dict)
        #     File "miniconda/3-4.5.4/envs/autotabular/lib/python3.7/multiprocessing/queues.py", line 87, in put  # noqa E501
        #     self._start_thread()
        #     File "miniconda/3-4.5.4/envs/autotabular/lib/python3.7/multiprocessing/queues.py", line 170, in _start_thread  # noqa E501
        #     self._thread.start()
        #     File "miniconda/3-4.5.4/envs/autotabular/lib/python3.7/threading.py", line 847, in start  # noqa E501
...
def get_cost_of_crash(metric: Scorer) -> float:
    ...
    # autotabular optimizes the err. This function translates
...
def _encode_exit_status(
        exit_status: Union[str, int, Type[BaseException]]) -> Union[str, int]:
    try:
...
class ExecuteTaFuncWithQueue(AbstractTAFunc):
    ...
    def __init__(
        ...
        **resampling_strategy_args: Any,
        ...
        if resampling_strategy == 'holdout':
            eval_function = autotabular.evaluation.train_evaluator.eval_holdout
        elif resampling_strategy == 'holdout-iterative-fit':
            eval_function = autotabular.evaluation.train_evaluator.eval_iterative_holdout
        elif resampling_strategy == 'cv-iterative-fit':
            eval_function = autotabular.evaluation.train_evaluator.eval_iterative_cv
        elif resampling_strategy == 'cv' or isinstance(
                resampling_strategy,
            (BaseCrossValidator, _RepeatedSplits, BaseShuffleSplit)):
            eval_function = autotabular.evaluation.train_evaluator.eval_cv
        elif resampling_strategy == 'partial-cv':
            eval_function = autotabular.evaluation.train_evaluator.eval_partial_cv
        elif resampling_strategy == 'partial-cv-iterative-fit':
            eval_function = autotabular.evaluation.train_evaluator.eval_partial_cv_iterative
        elif resampling_strategy == 'test':
            eval_function = autotabular.evaluation.test_evaluator.eval_t
        ...
        self.autotabular_seed = autotabular_seed
        ...
        if self.port is None:
            self.logger: Union[logging.Logger,
                               PickableLoggerAdapter] = logging.getLogger(
                                   'TAE')
        else:
    ...
    def run_wrapper(
        ...
        """wrapper function for ExecuteTARun.run_wrapper() to cap the target
        algorithm runtime if it would run over the total allowed runtime.
        ...
        config_id = (
            run_info.config
            if isinstance(run_info.config, int) else run_info.config.config_id)
    ...
    def run(
    ...
    ) -> Tuple[StatusType, float, float, Dict[str, Union[int, float, str, Dict,
                                                         List, Tuple]]]:
        ...
        if obj.exit_status in (pynisher.TimeoutException,
            ...
            try:
                info = autotabular.evaluation.util.read_queue(queue)
                result = info[-1]['loss']
        ...
        if (info is not None and self.resampling_strategy
            ...
            learning_curve = autotabular.evaluation.util.extract_learning_curve(
                info)
            learning_curve_runtime = autotabular.evaluation.util.extract_learning_curve(
                info, 'duration')
            ...
            train_learning_curve = autotabular.evaluation.util.extract_learning_curve(
                info, 'train_loss')
            ...
            if self._get_validation_loss:
                validation_learning_curve = autotabular.evaluation.util.extract_learning_curve(
                    info,
                    'validation_loss',
                )
            ...
            if self._get_test_loss:
                test_learning_curve = autotabular.evaluation.util.extract_learning_curve(
                    info,
                    'test_loss',
                )
        ...
        autotabular.evaluation.util.empty_queue(queue)
