...
import numpy as np
import pandas as pd
from autotabular.automl import AutoML, AutoMLClassifier, AutoMLRegressor
...
from autotabular.util.backend import create
...
class AutotabularEstimator(BaseEstimator):
    ...
    def __init__(self,
        ...
        tmp_folder : string, optional (None)
            ...
            automatically use ``/tmp/autotabular_tmp_$pid_$random_number``
        ...
        metadata_directory : str, optional (None)
            ...
            (autotabular.metalearning.files) is used.

        metric : Scorer, optional (None)
            An instance of :class:`autotabular.metrics.Scorer` as created by
            :meth:`autotabular.metrics.make_scorer`. These are the `Built-in
            Metrics`_.
    ...
    def fit_ensemble(self,
        ...
        task : int
            A constant from the module ``autotabular.constants``. Determines
    ...
    def leaderboard(
            ...
            include: Optional[Union[str,
                                    Iterable[str]]] = None) -> pd.DataFrame:
        ...
        * ``"model_id"`` - The id given to a model by ``autotabular``.
        ...
        * ``"feature_preprocessors"`` - The preprocessors for features types
        ...
        #      weights for.
        column_types = AutotabularEstimator._leaderboard_columns()

        # Validation of top_k
        if (not (isinstance(top_k, str) or isinstance(top_k, int))
        ...
        if not (isinstance(sort_order, str)
                and sort_order in valid_sort_orders):
        ...
        model_runs = {
            rval.additional_info['num_run']: {
                ...
                rval.cost,
                'train_loss':
                rval.additional_info['train_loss'] if has_key(
                    rval, 'train_loss') else None,
                'config_origin':
                rval.additional_info['configuration_origin'] if has_key(
                    rval, 'configuration_origin') else None
            }
        ...
        if model_type is None:
            raise RuntimeError(
                f'Unknown `automl_class` {self._get_automl_class()}')
        ...
        for model_id, run_info in model_runs.items():
            ...
            run_info.update({
                'balancing_strategy':
                run_config.get('balancing:strategy', None),
                'type':
                run_config[f'{model_type}:__choice__'],
                'data_preprocessors': [
        ...
        if 'rank' in columns and sort_by == 'cost':
            ...
            dataframe.sort_values(
                by=sort_by, ascending=ascending_param, inplace=True)
...
class AutotabularClassifier(AutotabularEstimator, ClassifierMixin):
...
class AutotabularRegressor(AutotabularEstimator, RegressorMixin):
    ...
    def fit(self,
        ...
        target_type = type_of_target(y)
        supported_types = [
            'continuous', 'binary', 'multiclass', 'continuous-multioutput'
        ]
        if target_type not in supported_types:
            raise ValueError(
