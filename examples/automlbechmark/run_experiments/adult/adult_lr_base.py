...
from autofe.feature_engineering.groupby import groupby_generate_feature, get_category_columns, get_numerical_columns
...
def get_groupby_total_data(df,
                            target_name,
                            threshold=0.9, 
                            k=5, 
                            methods=["min", "max", "sum", "mean", "std", "count"],
                            reserve=True):
    generated_feature = groupby_generate_feature(df, target_name, threshold, k, methods, reserve)
    return generated_feature


def get_GBDT_total_data(df,
                        target_name):
...
def get_nn_embedding_total_data(df,
                                target_name):
    target = df[target_name].values
    cat_col_names = get_category_columns(df, target_name)
    num_cols_names =  get_numerical_columns(df, target_name)
    label_encoder = LabelEncoder(cat_col_names)
    df_tans = label_encoder.fit_transform(df)
    ...
    X_wide = wide_prprocessor.fit_transform(df_tans)

    tab_preprocessor = TabPreprocessor(
        embed_cols=cat_col_names,
        continuous_cols=num_cols_names,
        for_transformer=True)
    X_tab = tab_preprocessor.fit_transform(df_tans)
...
def train_and_evaluate(total_data,
    ...
    pred = classfier.predict_proba(X_test)[:, 1]
...
if __name__ == "__main__":
    ...
    train_and_evaluate(total_data_embed, target_name, len_train, classfier)