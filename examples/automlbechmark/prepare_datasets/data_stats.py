...
adult_test = pd.read_csv(
    RAW_DATA_DIR / 'adult/adult.test', skiprows=1, names=colnames)
...
bank_marketing['target'] = (
    bank_marketing['y'].apply(lambda x: x == 'yes')).astype(int)
...
basic_stats_df['objective'] = ['binary_classification'] * 2 + ['regression'
                                                               ] * 2
